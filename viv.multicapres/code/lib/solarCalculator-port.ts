...
/// <reference path="../../../../../../../type-sandbox/src/main/ts/geo.GeoPoint.ts"/>
/// <reference path="../../../../../../../type-sandbox/src/main/ts/time.DateTime.ts"/>


function calculateSolarTimes(where:geo.GeoPoint, when:time.Date) {

    var lat = where.latitude;
    var lng = where.longitude;

    var reqDate = new dates.ZonedDateTime(where);
    ...
    var solarNoon:{date: {}, time: {hour: number, minute: number, second: number, timezone?: string}} = calcSolNoon(julianDay, lng, tz, dst)
...
function dayString(julianDay):date {
